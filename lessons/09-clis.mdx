...
title: CLIs
---
A CLI, or command line interface is a program designed to start and complete one-off
...
## Creating a CLI
Creating a CLI in Node.js just takes an extra step or two because
...
The first line in that file is called a shebang or hashbang. It's needed to tell
the machine where the interpreter is located that is needed to execute this file.
For us, that will be Node.js. 

Next we need to tell Node.js what the name of our CLI is so we can actually
...
Once installed, this package will have its bin command installed into your
